# Claude Code 错误率从 41% 到 3%：CLAUDE.md 到底改对了什么？

- 来源：微信公众号「架构师」（JiaGouX）
- 作者：若飞
- 发布日期：2026-05-11
- 原文链接：https://mp.weixin.qq.com/s/nAqv-0xAeoalLHS-nCt0TQ
- 摘要：把 Agent 昨天犯过的错，写成它明天开工前能读懂的约定。
...
我们这段时间一直在聊 Agentic Engineering、Harness、上下文工作集、长周期 Agent 和可验证交付。落到日常开发里，就是一句话：
...
这也补上了前面几篇没有展开的一小块。讲 Cursor 的 Harness 方法论时，我们把重点放在评估、观测、回滚和模型适配；讲 Martin Fowler 的 AI 研发提醒时，重点放在非确定性如何进入研发链路；讲长周期 Agent 时，重点放在目标、状态、验证和接管的证据链。
...
前面写《.claude 文件夹里到底应该放什么》时，我更关心的是东西应该放在哪里：哪些进 CLAUDE.md，哪些进 settings.json，哪些交给 hooks、commands、skills。
...
这轮讨论的源头，还是 Karpathy 早前对 Claude 写代码的几类吐槽。
...
![CLAUDE.md 的加载层级](data:image/svg+xml,...)
...
![Claude hooks 生命周期](data:image/svg+xml,...)
...
1. 拉一份最小模板，可以用 Forrest Chang 的模板，也可以用上面那份骨架。
2. 删到 120 行以内，只留当前项目真的用得上的规则。
3. 补 5 条项目命令：安装、类型检查、lint、局部测试、全量测试。
4. 补 5 条项目边界：危险目录、只读文件、禁止命令、数据 dry-run、测试跳过汇报。
5. 找一个真实小 bug 试跑，不要用玩具任务。
6. 看 Agent 有没有少问重复问题、少做无关改动、验证汇报有没有更清楚。
7. 只根据真实失败继续改，不因为"看起来更完整"继续加规则。
...
> 以下为作者「架构师」公众号文末附的相关文章链接，均为同一系列的若飞前作：
